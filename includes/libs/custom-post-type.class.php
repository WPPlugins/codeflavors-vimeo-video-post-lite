...
<div class="wrap">
	...
	<h2>
		<?php _e('Automatic import', 'codeflavors-vimeo-video-post-lite')?><sup><?php _e('PRO feature', 'codeflavors-vimeo-video-post-lite');?></sup>
		<a class="add-new-h2" href="#"><?php _e('Add New', 'codeflavors-vimeo-video-post-lite');?></a>	
	</h2>	
	<p>
		<?php _e('PRO feature that allows automatic creation of video posts from channels, albums, categories, groups or user uploads.', 'codeflavors-vimeo-video-post-lite');?>
	</p>
	<p>
		<?php _e('Import frequency can be customized to import batches of 20 videos at intervals between 1 minute and 1 day.', 'codeflavors-vimeo-video-post-lite');?>
	</p>
	<p>
...
<div class="wrap">
	...
	<h2><?php _e('Import videos', 'codeflavors-vimeo-video-post-lite')?></h2>
		...
		if( !$CVM_List_Table ){		
			...
			// meta boxes
			add_meta_box('cvm-import-feed-entries', __('Import', 'codeflavors-vimeo-video-post-lite'), array($this, 'import_entries_meta'), $page_hook, 'side');
			add_meta_box('cvm-import-new-category-metabox', 	__('Categories', 'codeflavors-vimeo-video-post-lite'), array($this, 'import_categories_meta'), $page_hook, 'side');
		?>
	...
	public function import_categories_meta(){
		...
		<div style="display:none;" id="cvm-add-video-category">			
			...
			<input type="button" id="cvm-new-category" name="cvm-new-category" class="secondary button" value="<?php _e('Create category', 'codeflavors-vimeo-video-post-lite');?>" />						
		</div>
		<?php 
	}
				...
				if( 'import' == $_REQUEST['action_top'] || 'import' == $_REQUEST['action2'] ){
					...
					if( $result ){					
						$response['success'] = sprintf( 
							__('Out of %d videos, %d were successfully imported and %d were skipped.', 'codeflavors-vimeo-video-post-lite'), 
							$result['total'],
							$result['imported'],
							$result['skipped']
						);
	...
	public function add_meta_boxes(){
		...
		foreach( $post_types as $type ){			
			...
			add_meta_box(
				'cvm-add-video', 
				__('Video shortcode', 'codeflavors-vimeo-video-post-lite'), 
	...
	public function post_shortcode_meta_box(){
		?>
		<p><?php _e('Add video/playlist into post.', 'codeflavors-vimeo-video-post-lite');?><p>
	...
	<tbody>
		<tr>
			...
			<td>
				...
				| <?php _e('Height', 'codeflavors-vimeo-video-post-lite');?> : <span class="cvm_height" id="cvm_calc_height"><?php echo cvm_player_height( $settings['aspect_ratio'], $settings['width'] );?></span>px
			</td>
		</tr>
