...
class CVM_Video_Import_List_Table extends WP_List_Table{
	...
	function column_title( $item ){	
		...
		// row actions
    	$actions = array(
    		'view' 		=> sprintf( '<a href="http://vimeo.com/%1$s" target="_cvm_vimeo_open">%2$s</a>', $item['video_id'], __('View on Vimeo', 'codeflavors-vimeo-video-post-lite') ),
    	);
    ...
    function no_items(){
    	if( is_wp_error( $this->query_errors ) ){
    		echo __( 'Query to Vimeo returned this error: ', 'codeflavors-vimeo-video-post-lite' ) . $this->query_errors->get_error_message();
    	}
    }
